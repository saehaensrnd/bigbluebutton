...
async function main() {
	...
	var recDuration = await page.$eval('#vjs_video_3_html5_api', el => el.duration);
     
        // If duration was set to 0 or is greater than recDuration, use recDuration value
        if(duration == 0 || duration > recDuration){
            duration = Math.ceil(recDuration);
        }

        await page.waitForSelector('.vjs-play-control');
	await page.$eval('.vjs-control', element => element.style.opacity = "0");
        await page.click('.vjs-play-control', {waitUntil: 'domcontentloaded'});
